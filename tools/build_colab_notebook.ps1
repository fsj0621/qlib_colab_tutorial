...
with R.start(experiment_name="backtest_analysis"):
    recorder = R.get_recorder()
    ba_rid = recorder.id
    # PortAnaRecord 的依赖检查沿用 SignalRecord.list()，会同时检查
    # pred.pkl 与 label.pkl；这里保存 infer 标签以满足检查，无需恢复 raw 数据。
    recorder.save_objects(**{"pred.pkl": pred_df, "label.pkl": label_df})
    print(
        f"预测与标签已保存：{len(pred_df):,} 行预测，{len(label_df):,} 行标签，"
        f"{len(backtest_codes)} 只股票。",
        flush=True,
    )
    ...
    if backtest_artifacts is None:
        raise RuntimeError(
            "PortAnaRecord 未生成结果；请确认当前 recorder 同时包含 pred.pkl 和 label.pkl。"
        )
