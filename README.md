...
`v1.0.6` 采用课堂轻量流程：raw / infer / learn 由一个月的小型 Alpha158 处理器演示，随后整体释放；正式处理器启用 `drop_raw=True`，使用 2017 年训练、2018 年验证和 2019–2020 年测试。训练和信号生成复用同一套模型与 `DatasetH`；`SignalRecord` 保存一次预测后即释放完整特征数据，`PortAnaRecord` 通过 `<PRED>` 直接读取 `pred.pkl`，不再重复预测。
