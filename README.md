...
`v1.0.5` 采用课堂轻量流程：raw / infer / learn 由一个月的小型 Alpha158 处理器演示，随后整体释放；正式处理器启用 `drop_raw=True`，使用 2017 年训练、2018 年验证和 2019–2020 年测试。训练复用一个 `DatasetH`，保存模型前释放 LightGBM 训练矩阵。
